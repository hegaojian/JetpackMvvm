...
import me.hgj.mvvm_nb_demo.app.weight.banner.HomeBannerViewHolder
...
class HomeFragment : BaseFragment<HomeViewModel, FragmentHomeBinding>() {
    //适配器
    private val articleAdapter: AriticleAdapter by lazy { AriticleAdapter(arrayListOf(), true).apply {
    ...
    override fun createObserver() {
        mViewModel.run {
            //监听首页文章列表请求的数据变化
            homeDataState.observe(this@HomeFragment, Observer {
            ...
            //监听轮播图请求的数据变化
            bannerData.observe(this@HomeFragment, Observer { viewState ->
            ...
            collectUiState.observe(this@HomeFragment, Observer {
                if (it.isSuccess) {
        ...
        appViewModel.run {
            ...
            userinfo.observe(this@HomeFragment, Observer {
                if (it != null) {
            ...
            //监听全局的主题颜色改变
            appColor.observe(this@HomeFragment, Observer {
                setUiTheme(it, listOf(toolbar, floatbtn, swipeRefresh, loadsir, footView))
            })
            //监听全局的列表动画改编
            appAnimation.observe(this@HomeFragment, Observer {
                articleAdapter.setAdapterAnimion(it)
            })
            //监听全局的收藏信息 收藏的Id跟本列表的数据id匹配则需要更新
            collect.observe(this@HomeFragment, Observer {
